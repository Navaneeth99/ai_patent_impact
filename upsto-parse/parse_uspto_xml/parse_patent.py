...
def _ensure_root_stdout_handler() -> None:
...
_ensure_root_stdout_handler()
logger = logging.getLogger(__name__)


# --- Plain text from patent XML ---


def element_to_plain_text(element: Tag | None, separator: str = "\n") -> str:
    if element is None:
        return ""
...
def extract_abstract_texts(application: Tag) -> list[str]:
    el = application.find("abstract")
    t = element_to_plain_text(el)
    return [t] if t else []


def extract_description_texts(application: Tag) -> list[str]:
    el = application.find("description")
    t = element_to_plain_text(el)
    return [t] if t else []


def extract_claim_texts(application: Tag) -> list[str]:
    claims_root = application.find("claims")
    if claims_root:
        claim_nodes = claims_root.find_all("claim", recursive=False)
    else:
        claim_nodes = application.find_all("claim")
    return [element_to_plain_text(el) for el in claim_nodes if element_to_plain_text(el)]
...
def build_full_text(
    ...
    parts: list[str] = []
    title = (publication_title or "").strip()
    if title:
...
# --- USPTO Open Data Portal bulk download ---

DEFAULT_API_ROOT = "https://api.uspto.gov/api/v1"


class USPTOAPIError(RuntimeError):
    """Raised when the ODP API returns an error or unexpected payload."""


def _request_json(url: str, api_key: str, *, timeout: float = 120.0) -> Any:
    ...
    try:
        ...
        raise USPTOAPIError(f"HTTP {e.code} for {url}: {detail}") from e
    except urllib.error.URLError as e:
        raise USPTOAPIError(f"Request failed for {url}: {e}") from e
    try:
        return json.loads(body)
    except json.JSONDecodeError as e:
        raise USPTOAPIError(f"Invalid JSON from {url}") from e
...
def list_product_files(
    ...
    *,
    ...
    if not key:
        raise USPTOAPIError("Missing API key: pass api_key= or set USPTO_API_KEY")
    ...
    url = f"{api_root.rstrip('/')}/datasets/products/{urllib.parse.quote(product)}?{q}"
    data = _request_json(url, key, timeout=timeout)
    if not isinstance(data, dict):
        raise USPTOAPIError(f"Expected JSON object from product listing, got {type(data)}")
    return data


def _walk_zip_urls(obj: Any, out: list[str]) -> None:
    if isinstance(obj, str):
        lower = obj.lower()
        if obj.startswith("http") and lower.endswith(".zip"):
            out.append(obj)
    elif isinstance(obj, dict):
        for v in obj.values():
            _walk_zip_urls(v, out)
    elif isinstance(obj, list):
        for v in obj:
            _walk_zip_urls(v, out)


def iter_zip_download_urls(payload: dict[str, Any] | list[Any]) -> Iterator[str]:
    found: list[str] = []
    _walk_zip_urls(payload, found)
    seen: set[str] = set()
    for u in found:
        if u not in seen:
            seen.add(u)
...
def download_url_to_path(
    ...
    *,
    ...
    key = api_key or os.environ.get("USPTO_API_KEY") or ""
    dest = Path(dest_dir)
...
def parse_uspto_file(bs, keep_log: bool = False):
    ...
    abstracts = extract_abstract_texts(bs)
    descriptions = extract_description_texts(bs)
    claims = extract_claim_texts(bs)
    full_text = build_full_text(publication_title, abstracts, descriptions, claims)
    ...
    if keep_log:
        ...
        preview = full_text[:8000] + ("..." if len(full_text) > 8000 else "")
        print(preview)
...
def _cmd_download(args: argparse.Namespace) -> int:
    ...
    urls = list(iter_zip_download_urls(payload))
    if not urls:
        logger.error("No .zip URLs found in API response (schema may have changed).")
        return 2
    ...
    for i, url in enumerate(urls):
        if args.max_files is not None and i >= args.max_files:
            break
        download_url_to_path(url, args.output_dir, api_key=args.api_key)
    return 0
...
def _cmd_parse(args: argparse.Namespace) -> int:
    if args.jsonl:
        push_to = args.jsonl
        push_fn = get_dump_function(push_to)
    else:
        db = PGDBInterface(config_file=args.postgres_config, silent_logging=True)
        push_fn = get_dump_function(
            db,
            patent_table_name=args.table,
            include_referential=args.referential,
        )

    load_local_files(
        dirpath_list=args.paths,
        push_to_func=push_fn,
        batch_size=args.batch_size,
        limit_per_file=args.limit_per_file,
        keep_log=args.verbose,
    )
    return 0
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="USPTO patent application XML: download APPXML zips or parse local .xml",
    )
    sub = parser.add_subparsers(dest="command", required=False)

    p_dl = sub.add_parser("download", help="List/download APPXML (or other product) zips via ODP API")
    p_dl.add_argument("--product", default="APPXML", help="Dataset product code (default: APPXML)")
    p_dl.add_argument("--from-date", required=True, dest="from_date", help="fileDataFromDate YYYY-MM-DD")
    p_dl.add_argument("--to-date", required=True, dest="to_date", help="fileDataToDate YYYY-MM-DD")
    p_dl.add_argument("--output-dir", default=".", help="Directory for downloaded .zip files")
    p_dl.add_argument(
        "--api-key",
        default=os.environ.get("USPTO_API_KEY"),
        help="USPTO API key (default: env USPTO_API_KEY)",
    )
    p_dl.add_argument("--api-root", default=DEFAULT_API_ROOT, help="API base URL")
    p_dl.add_argument(
        "--print-urls",
        action="store_true",
        help="Only print discovered zip URLs; do not download",
    )
    p_dl.add_argument("--max-files", type=int, default=None, help="Download at most N zips")
    p_dl.set_defaults(func=_cmd_download)

    p_parse = sub.add_parser("parse", help="Parse local .xml files (batched multi-document weeklies)")
    p_parse.add_argument("paths", nargs="+", help="XML files and/or directories")
    p_parse.add_argument("--batch-size", type=int, default=50)
    p_parse.add_argument("--limit-per-file", type=int, default=None)
    p_parse.add_argument("--verbose", action="store_true")
    p_parse.add_argument(
        "--jsonl",
        metavar="FILE",
        help="Append JSON lines to this file instead of PostgreSQL",
    )
    p_parse.add_argument(
        "--postgres-config",
        default="config/postgres.tsv",
        help="TSV config when not using --jsonl",
    )
    p_parse.add_argument("--table", default="uspto_patents")
    p_parse.set_defaults(referential=True)
    p_parse.add_argument(
        "--no-referential",
        action="store_false",
        dest="referential",
        help="Do not insert into uspto_referential_documents",
    )
...
def main(argv: list[str] | None = None) -> int:
    """CLI: ``download`` / ``parse`` subcommands, or legacy bare filenames (PostgreSQL)."""
    argv = list(sys.argv[1:] if argv is None else argv)

    if argv and argv[0] in ("download", "parse"):
    ...
    # Legacy: python parse_patent.py <xml paths> → Postgres
    if not argv:
        return 1
    ...
    _push_to_func = get_dump_function(
        _db,
        patent_table_name="uspto_patents",
